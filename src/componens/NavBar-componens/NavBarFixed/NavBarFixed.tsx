...
const NavBarFixed = () => {
    ...
    const {menuBurgerButtonChange, findProductsInput} = useActions()
    const {menu_button_button} = useTypedSelector(state => state.menuBurger)
    const {input, loading, products} = useTypedSelector(state => state.findProducts)
    ...
    return (
       <nav className={`nav-bar-fixed ${scroll >=140 ? "active": ''}`} >
          <div className="container">
              <div className="nav-content">
                  ...
                  <div className="nav-items-products-search-basket">
                      <div className="nav-items-search-modal-menu">
                          <div className="nav-items-search">
                              <input className={'nav-search-input'} value={input} onChange={(event)=> {
                                  findProductsInput(event)
                              }}  type="text"/>
                              <div className={`nav-search-modal-menu ${input.length > 1 ? "active": ""}`} >
                                  {loading ? <Spinner/> :
                                      products.length > 0 ?
                                          <div className={'find-products'} >{ products.map(product => <FindProduct img={product.img} productName={product.productName } id={product._id} />)}</div>
                                          : "Empty"
                                  }
                      ...
                      <NavLink to={"/basket"}>
                          <div className="nav-items-bscket">
                              <div className="bscket">
                                  <svg width="22" height="20" viewBox="0 0 27 24" fill="none" xmlns="http://www.w3.org/2000/svg">
